...
#if defined(CONFIG_LCM_H231_M119_CHECK)
extern void lcm_reload(void);
extern unsigned int gc9503v_flag;
#endif
...
#if defined(CONFIG_LED_SET_GPIO_NUM)
int my_mt_brightness_set_gpio(struct cust_mt65xx_led *cust , int level);
#endif
...
#if defined(CONFIG_LED_SET_GPIO_NUM)
		return my_mt_brightness_set_gpio(cust, level);
#else
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	#if defined(CONFIG_LCM_H231_M119_CHECK)
		char *p;
		p = strstr(saved_command_line, "lcm=1-GC9503V_YOUYI_040WVA070_MIPI2_WVGA");

	if (p != NULL)
	{

			if((led_data->level <= 0)&& (gc9503v_flag==1))
			{
				lcm_reload();
				gc9503v_flag=0;
				mdelay(50);
...
int my_mt_brightness_set_gpio(struct cust_mt65xx_led *cust , int level)
{
	...
	int gpio_num = 0;
	int ret = 0;
	char request_name[32] = "gpio_led_";
	char node_name[32] = "mediatek,";
		
	led_node = of_find_compatible_node(NULL, NULL,strncat(node_name, cust->name,sizeof(node_name) - strlen(node_name) - 1));
	if (!led_node) {
		pr_err("Cannot find led node from dts\n");
	}
	gpio_num = of_get_named_gpio(led_node, "gpio", 0);
	if (gpio_num < 0) {
		pr_err("No %s led_gpio info!\n", cust->name);
		return gpio_num;
	}
	strncat(request_name, cust->name,sizeof(request_name) - strlen(request_name) - 1);
	pr_err("led gpio mode gpio = %d %s\n", gpio_num,request_name);
	
	ret = gpio_request(gpio_num, request_name);
	if (ret < 0)
		pr_err("Unable to request %s\n",request_name);

	ret = gpio_direction_output(gpio_num, !!level);
	if (ret) {
		pr_err("led gpio gpio_direction_output error\r\n");
	}else{
		gpio_free(gpio_num);
	}
	return 0;
