...
#define SIOSENSOR_INFO(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)
/****************************   Modify end    *******************************************/

#ifdef CONFIG_BIRD_FAKE_DUAL_CAM_SUPPORT
extern kal_uint32 mainsensor_id;
extern kal_uint32 fake_dualsensor_id;
#ifdef CONFIG_BIRD_FAKE_DUAL_CAM_SP0A08
extern kal_uint32 return_sensor_id_sp0a08(void);
extern void sensor_init_sp0a08(void);
#endif
#endif
...
static imgsensor_info_struct imgsensor_info = { 
  ...
  .custom5 = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,//sensor output first pixel color SENSOR_OUTPUT_FORMAT_RAW_Gb B gr
...
{
	SIOSENSOR_INFO("dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	SIOSENSOR_INFO("framerate = %d, min framelength should enable(%d) \n", framerate,min_framelength_en);
...
static void write_shutter(kal_uint16 shutter)
{
	...
	SIOSENSOR_INFO("Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
  ...
  //gain = 64 = 1x real gain.
	kal_uint16 reg_gain;
SIOSENSOR_INFO("set_gain %d \n", gain);
	if (gain < BASEGAIN || gain > 16 * BASEGAIN) {
		SIOSENSOR_INFO("Error gain setting");
		if (gain < BASEGAIN)
			gain = BASEGAIN;
	...
	spin_unlock(&imgsensor_drv_lock);
	SIOSENSOR_INFO("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
{
	SIOSENSOR_INFO("le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	switch (image_mirror) {
		case IMAGE_NORMAL:
			write_cmos_sensor(0x0000,(0X00&0xff)<<8); //GR
			break;
		case IMAGE_H_MIRROR:
			write_cmos_sensor(0x0000,(0X01&0xff)<<8); //R
			break;
		case IMAGE_V_MIRROR:
			write_cmos_sensor(0x0000,(0X02&0xff)<<8); //B	
			break;
		case IMAGE_HV_MIRROR:
			write_cmos_sensor(0x0000,(0X03&0xff)<<8); //GB
			break;
		default:
			SIOSENSOR_INFO("Error image_mirror setting\n");
	}
...
////////////////////////////////////////////////
	...
	write_cmos_sensor(0x0000, 0x0200); //image orient 0x0100  
...
#ifdef CONFIG_BIRD_FAKE_DUAL_CAM_SUPPORT
					mainsensor_id = imgsensor_info.sensor_id;
#endif        
				SIOSENSOR_INFO("i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
                return ERROR_NONE;
            }
			SIOSENSOR_INFO("Read sensor id fail, i2c write id: 0x%x, ReadOut sensor id: 0x%x, imgsensor_info.sensor_id:0x%x.\n", imgsensor.i2c_write_id,*sensor_id,imgsensor_info.sensor_id);	
            retry--;
        } while(retry > 0);
        i++;
        retry = 1;
...
static kal_uint32 open(void)
{
    ...
    //sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
#ifdef CONFIG_BIRD_FAKE_DUAL_CAM_SP0A08
		fake_dualsensor_id = return_sensor_id_sp0a08();
		printk("fake_dualsensor_id = %d\n",fake_dualsensor_id);
		if(fake_dualsensor_id == 0xA2)
		{
			sensor_init_sp0a08();
			printk("main2 init \n");
	}
		else
		printk("fake_dualsensor_id read id failed\n");
#endif
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        spin_lock(&imgsensor_drv_lock);
        imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
        ...
        do {
            ...
            SIOSENSOR_INFO("Read sensor id fail, id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	preview_setting();
	//set_mirror_flip(IMAGE_NORMAL);   //zhuying
	mdelay(10);
	return ERROR_NONE;
}	/*	preview   */
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		SIOSENSOR_INFO("capture30fps: use cap30FPS's setting: %d fps!\n",imgsensor.current_fps/10);
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		SIOSENSOR_INFO("cap115fps: use cap1's setting: %d fps!\n",imgsensor.current_fps/10);
		...
		SIOSENSOR_INFO("Warning:=== current_fps %d fps is not support, so use cap1's setting\n",imgsensor.current_fps/10);
		imgsensor.pclk = imgsensor_info.cap1.pclk;
		imgsensor.line_length = imgsensor_info.cap1.linelength;
		imgsensor.frame_length = imgsensor_info.cap1.framelength;  
		imgsensor.min_frame_length = imgsensor_info.cap1.framelength;
...
{
	SIOSENSOR_INFO("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	SIOSENSOR_INFO("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
		...
		default:  //coding with  preview scenario by default
			...
			imgsensor.min_frame_length = imgsensor.frame_length;
			spin_unlock(&imgsensor_drv_lock);
			SIOSENSOR_INFO("error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}	
	return ERROR_NONE;
}
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
		 ...
		 write_cmos_sensor(0x020a, 0x000a);
		 write_cmos_sensor(0x020c, 0x0a0a);
		 SIOSENSOR_INFO(">>crc enable>> reg: %d\n", enable);
	} else {
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            SIOSENSOR_INFO("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
