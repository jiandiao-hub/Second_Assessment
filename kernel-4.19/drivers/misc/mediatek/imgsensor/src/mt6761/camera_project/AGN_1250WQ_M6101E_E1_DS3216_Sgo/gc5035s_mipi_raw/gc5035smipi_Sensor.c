...
#include "gc5035smipi_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc5035s_camera_sensor"
#define LOG_1 cam_pr_debug("GC5035SMIPI, 2LANE\n")
/********************   Modify end    *********************************/
...
static kal_uint32 Dgain_ratio = GC5035S_SENSOR_DGAIN_BASE;
static struct gc5035s_otp_t gc5035s_otp_data;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035S_SENSOR_ID,
...
static kal_uint8 gc5035s_otp_read_byte(kal_uint16 addr)
{
...
static void gc5035s_otp_read_group(kal_uint16 addr,
	kal_uint8 *data, kal_uint16 length)
{
	kal_uint16 i = 0;

	if ((((addr & 0x1fff) >> 3) + length) > GC5035S_OTP_DATA_LENGTH) {
...
static void gc5035s_gcore_read_dpc(void)
{
	kal_uint8 dpcFlag = 0;
	struct gc5035s_dpc_t *pDPC = &gc5035s_otp_data.dpc;

	dpcFlag = gc5035s_otp_read_byte(GC5035S_OTP_DPC_FLAG_OFFSET);
	cam_pr_debug("dpc flag = 0x%x\n", dpcFlag);
	switch (GC5035S_OTP_GET_2BIT_FLAG(dpcFlag, 0)) {
	case GC5035S_OTP_FLAG_EMPTY: {
		cam_pr_debug("dpc info is empty!!\n");
		pDPC->flag = GC5035S_OTP_FLAG_EMPTY;
		break;
	}
	case GC5035S_OTP_FLAG_VALID: {
		cam_pr_debug("dpc info is valid!\n");
		pDPC->total_num =
			gc5035s_otp_read_byte(GC5035S_OTP_DPC_TOTAL_NUMBER_OFFSET)
			+
			gc5035s_otp_read_byte(
			GC5035S_OTP_DPC_ERROR_NUMBER_OFFSET);
		pDPC->flag = GC5035S_OTP_FLAG_VALID;
		cam_pr_debug("total_num = %d\n", pDPC->total_num);
		break;
	}
	default:
		pDPC->flag = GC5035S_OTP_FLAG_INVALID;
		break;
	}
}

static void gc5035s_gcore_read_reg(void)
{
	kal_uint8 i = 0;
	kal_uint8 j = 0;
	kal_uint16 base_group = 0;
	kal_uint8 reg[GC5035S_OTP_REG_DATA_SIZE];
	struct gc5035s_reg_update_t *pRegs = &gc5035s_otp_data.regs;

	memset(&reg, 0, GC5035S_OTP_REG_DATA_SIZE);
	pRegs->flag = gc5035s_otp_read_byte(GC5035S_OTP_REG_FLAG_OFFSET);
	cam_pr_debug("register update flag = 0x%x\n", pRegs->flag);
	if (pRegs->flag == GC5035S_OTP_FLAG_VALID) {
		gc5035s_otp_read_group(GC5035S_OTP_REG_DATA_OFFSET,
				&reg[0], GC5035S_OTP_REG_DATA_SIZE);

		for (i = 0; i < GC5035S_OTP_REG_MAX_GROUP; i++) {
			base_group = i * GC5035S_OTP_REG_BYTE_PER_GROUP;
			for (j = 0; j < GC5035S_OTP_REG_REG_PER_GROUP; j++)
				if (GC5035S_OTP_CHECK_1BIT_FLAG(reg[base_group],
						(4 * j + 3))) {
...
#if GC5035S_OTP_FOR_CUSTOMER
static kal_uint8 gc5035s_otp_read_module_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 module_start_offset = GC5035S_OTP_MODULE_DATA_OFFSET;
	kal_uint8 info[GC5035S_OTP_MODULE_DATA_SIZE];
	struct gc5035s_module_info_t module_info = { 0 };

	memset(&info, 0, GC5035S_OTP_MODULE_DATA_SIZE);
	memset(&module_info, 0, sizeof(struct gc5035s_module_info_t));

	flag = gc5035s_otp_read_byte(GC5035S_OTP_MODULE_FLAG_OFFSET);
	cam_pr_debug("flag = 0x%x\n", flag);

	for (idx = 0; idx < GC5035S_OTP_GROUP_CNT; idx++) {
		switch (GC5035S_OTP_GET_2BIT_FLAG(flag, 2 * (1 - idx))) {
		case GC5035S_OTP_FLAG_EMPTY: {
		...
		case GC5035S_OTP_FLAG_VALID: {
			...
			module_start_offset = GC5035S_OTP_MODULE_DATA_OFFSET
				+ GC5035S_OTP_GET_OFFSET(idx *
				GC5035S_OTP_MODULE_DATA_SIZE);
			gc5035s_otp_read_group(module_start_offset,
				&info[0], GC5035S_OTP_MODULE_DATA_SIZE);
			for (i = 0; i < GC5035S_OTP_MODULE_DATA_SIZE - 1; i++)
				check += info[i];

			if ((check % 255 + 1) ==
					info[GC5035S_OTP_MODULE_DATA_SIZE - 1]) {
		...
		case GC5035S_OTP_FLAG_INVALID:
		case GC5035S_OTP_FLAG_INVALID2: {
...
static void gc5035s_otp_read_wb_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	...
	kal_uint16 wb_start_offset = GC5035S_OTP_WB_DATA_OFFSET;
	kal_uint16 golden_start_offset = GC5035S_OTP_GOLDEN_DATA_OFFSET;
	kal_uint8 wb[GC5035S_OTP_WB_DATA_SIZE];
	kal_uint8 golden[GC5035S_OTP_GOLDEN_DATA_SIZE];
	struct gc5035s_wb_t *pWB = &gc5035s_otp_data.wb;
	struct gc5035s_wb_t *pGolden = &gc5035s_otp_data.golden;

	memset(&wb, 0, GC5035S_OTP_WB_DATA_SIZE);
	memset(&golden, 0, GC5035S_OTP_GOLDEN_DATA_SIZE);
	flag = gc5035s_otp_read_byte(GC5035S_OTP_WB_FLAG_OFFSET);
	cam_pr_debug("flag = 0x%x\n", flag);
	...
	for (idx = 0; idx < GC5035S_OTP_GROUP_CNT; idx++) {
		...
		case GC5035S_OTP_FLAG_EMPTY: {
			...
			pWB->flag = pWB->flag | GC5035S_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035S_OTP_FLAG_VALID: {
			cam_pr_debug("wb group %d is valid!\n", idx + 1);
			wb_start_offset = GC5035S_OTP_WB_DATA_OFFSET
				+ GC5035S_OTP_GET_OFFSET(idx *
				GC5035S_OTP_WB_DATA_SIZE);
			gc5035s_otp_read_group(wb_start_offset, &wb[0],
				GC5035S_OTP_WB_DATA_SIZE);

			for (i = 0; i < GC5035S_OTP_WB_DATA_SIZE - 1; i++)
				wb_check += wb[i];

			if ((wb_check % 255 + 1) ==
					wb[GC5035S_OTP_WB_DATA_SIZE - 1]) {
				pWB->rg = (wb[0] | ((wb[1] & 0xf0) << 4));
				pWB->bg = (((wb[1] & 0x0f) << 8) | wb[2]);
				pWB->rg = pWB->rg == 0 ?
					GC5035S_OTP_WB_RG_TYPICAL : pWB->rg;
				pWB->bg = pWB->bg == 0 ?
					GC5035S_OTP_WB_BG_TYPICAL : pWB->bg;
				pWB->flag = pWB->flag | GC5035S_OTP_FLAG_VALID;
				cam_pr_debug("wb r/g = 0x%x\n", pWB->rg);
				cam_pr_debug("wb b/g = 0x%x\n", pWB->bg);
			} else {
				pWB->flag = pWB->flag | GC5035S_OTP_FLAG_INVALID;
		...
		switch (GC5035S_OTP_GET_2BIT_FLAG(flag, 2 * (3 - idx))) {
		case GC5035S_OTP_FLAG_EMPTY: {
			cam_pr_debug("golden group %d is empty!\n", idx + 1);
			pGolden->flag = pGolden->flag | GC5035S_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035S_OTP_FLAG_VALID: {
			cam_pr_debug("golden group %d is valid!\n", idx + 1);
			golden_start_offset = GC5035S_OTP_GOLDEN_DATA_OFFSET
				+ GC5035S_OTP_GET_OFFSET(idx *
				GC5035S_OTP_GOLDEN_DATA_SIZE);
			gc5035s_otp_read_group(golden_start_offset,
				&golden[0], GC5035S_OTP_GOLDEN_DATA_SIZE);
			for (i = 0; i < GC5035S_OTP_GOLDEN_DATA_SIZE - 1; i++)
				golden_check += golden[i];

			if ((golden_check % 255 + 1) ==
				golden[GC5035S_OTP_GOLDEN_DATA_SIZE - 1]) {
				pGolden->rg = (golden[0] | ((golden[1] &
						0xf0) << 4));
				pGolden->bg = (((golden[1] & 0x0f) << 8) |
						golden[2]);
				pGolden->rg = pGolden->rg == 0 ?
					GC5035S_OTP_WB_RG_TYPICAL : pGolden->rg;
				pGolden->bg = pGolden->bg == 0 ?
					GC5035S_OTP_WB_BG_TYPICAL : pGolden->bg;
				pGolden->flag = pGolden->flag |
					GC5035S_OTP_FLAG_VALID;
		...
		case GC5035S_OTP_FLAG_INVALID2: {
			...
			pGolden->flag = pGolden->flag | GC5035S_OTP_FLAG_INVALID;
			break;
		}
		default:
			break;
...
static kal_uint8 gc5035s_otp_read_sensor_info(void)
{
	kal_uint8 moduleID = 0;
#if GC5035S_OTP_DEBUG
	kal_uint16 i = 0;
	kal_uint8 debug[GC5035S_OTP_DATA_LENGTH];
#endif

	gc5035s_gcore_read_dpc();
	gc5035s_gcore_read_reg();
#if GC5035S_OTP_FOR_CUSTOMER
	moduleID = gc5035s_otp_read_module_info();
	gc5035s_otp_read_wb_info();

#endif

#if GC5035S_OTP_DEBUG
	memset(&debug[0], 0, GC5035S_OTP_DATA_LENGTH);
	gc5035s_otp_read_group(GC5035S_OTP_START_ADDR, &debug[0],
			GC5035S_OTP_DATA_LENGTH);
	for (i = 0; i < GC5035S_OTP_DATA_LENGTH; i++)
		cam_pr_debug("addr = 0x%x, data = 0x%x\n",
				GC5035S_OTP_START_ADDR + i * 8, debug[i]);
#endif

	return moduleID;
}

static void gc5035s_otp_update_dd(void)
{
	kal_uint8 state = 0;
	kal_uint8 n = 0;
	struct gc5035s_dpc_t *pDPC = &gc5035s_otp_data.dpc;

	if (pDPC->flag == GC5035S_OTP_FLAG_VALID) {
...
#if GC5035S_OTP_FOR_CUSTOMER
static void gc5035s_otp_update_wb(void)
{
	kal_uint16 r_gain = GC5035S_OTP_WB_GAIN_BASE;
	kal_uint16 g_gain = GC5035S_OTP_WB_GAIN_BASE;
	kal_uint16 b_gain = GC5035S_OTP_WB_GAIN_BASE;
	kal_uint16 base_gain = GC5035S_OTP_WB_CAL_BASE;
	kal_uint16 r_gain_curr = GC5035S_OTP_WB_CAL_BASE;
	kal_uint16 g_gain_curr = GC5035S_OTP_WB_CAL_BASE;
	kal_uint16 b_gain_curr = GC5035S_OTP_WB_CAL_BASE;
	kal_uint16 rg_typical = GC5035S_OTP_WB_RG_TYPICAL;
	kal_uint16 bg_typical = GC5035S_OTP_WB_BG_TYPICAL;
	struct gc5035s_wb_t *pWB = &gc5035s_otp_data.wb;
	struct gc5035s_wb_t *pGolden = &gc5035s_otp_data.golden;

	if (GC5035S_OTP_CHECK_1BIT_FLAG(pGolden->flag, 0)) {
		rg_typical = pGolden->rg;
		bg_typical = pGolden->bg;
	} else {
		rg_typical = GC5035S_OTP_WB_RG_TYPICAL;
		bg_typical = GC5035S_OTP_WB_BG_TYPICAL;
	}
	cam_pr_debug("typical rg = 0x%x, bg = 0x%x\n", rg_typical, bg_typical);

	if (GC5035S_OTP_CHECK_1BIT_FLAG(pWB->flag, 0)) {
		r_gain_curr = GC5035S_OTP_WB_CAL_BASE * rg_typical / pWB->rg;
		b_gain_curr = GC5035S_OTP_WB_CAL_BASE * bg_typical / pWB->bg;
		g_gain_curr = GC5035S_OTP_WB_CAL_BASE;

		base_gain = (r_gain_curr < b_gain_curr) ?
			r_gain_curr : b_gain_curr;
		base_gain = (base_gain < g_gain_curr) ? base_gain : g_gain_curr;

		r_gain = GC5035S_OTP_WB_GAIN_BASE * r_gain_curr / base_gain;
		g_gain = GC5035S_OTP_WB_GAIN_BASE * g_gain_curr / base_gain;
		b_gain = GC5035S_OTP_WB_GAIN_BASE * b_gain_curr / base_gain;
...
static void gc5035s_otp_update_reg(void)
{
	kal_uint8 i = 0;

	cam_pr_debug("reg count = %d\n", gc5035s_otp_data.regs.cnt);

	if (GC5035S_OTP_CHECK_1BIT_FLAG(gc5035s_otp_data.regs.flag, 0))
		for (i = 0; i < gc5035s_otp_data.regs.cnt; i++) {
			write_cmos_sensor(0xfe,
					gc5035s_otp_data.regs.reg[i].page);
			write_cmos_sensor(gc5035s_otp_data.regs.reg[i].addr,
					gc5035s_otp_data.regs.reg[i].value);
			cam_pr_debug("reg[%d] P%d:0x%x -> 0x%x\n", i,
				gc5035s_otp_data.regs.reg[i].page,
				gc5035s_otp_data.regs.reg[i].addr,
				gc5035s_otp_data.regs.reg[i].value);
		}
}

static void gc5035s_otp_update(void)
{
	gc5035s_otp_update_dd();
#if GC5035S_OTP_FOR_CUSTOMER
	gc5035s_otp_update_wb();
#endif
	gc5035s_otp_update_reg();
}

static kal_uint8 gc5035s_otp_identify(void)
{
	kal_uint8 moduleID = 0;

	memset(&gc5035s_otp_data, 0, sizeof(gc5035s_otp_data));
	...
	gc5035s_otp_read_group(GC5035S_OTP_ID_DATA_OFFSET,
			&gc5035s_otp_data.otp_id[0], GC5035S_OTP_ID_SIZE);
	moduleID = gc5035s_otp_read_sensor_info();
...
static void gc5035s_otp_function(void)
{
	kal_uint8 i = 0, flag = 0;
	kal_uint8 otp_id[GC5035S_OTP_ID_SIZE];

	memset(&otp_id, 0, GC5035S_OTP_ID_SIZE);
	...
	gc5035s_otp_read_group(GC5035S_OTP_ID_DATA_OFFSET,
			&otp_id[0], GC5035S_OTP_ID_SIZE);
	for (i = 0; i < GC5035S_OTP_ID_SIZE; i++)
		if (otp_id[i] != gc5035s_otp_data.otp_id[i]) {
			flag = 1;
			break;
		}
	...
	if (flag == 1) {
		...
		memset(&gc5035s_otp_data, 0, sizeof(gc5035s_otp_data));
		for (i = 0; i < GC5035S_OTP_ID_SIZE; i++)
			gc5035s_otp_data.otp_id[i] = otp_id[i];
		gc5035s_otp_read_sensor_info();
	}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC5035S_SENSOR_GAIN_BASE)
		reg_gain = GC5035S_SENSOR_GAIN_BASE;
	else if (reg_gain > GC5035S_SENSOR_GAIN_MAX)
		reg_gain = GC5035S_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC5035S_AGC_Param[GC5035S_SENSOR_GAIN_MAP_SIZE][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
	...
	for (gain_index = GC5035S_SENSOR_GAIN_MAX_VALID_INDEX - 1;
			gain_index > 0; gain_index--)
		if (reg_gain >= GC5035S_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC5035S_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC5035S_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	cam_pr_debug("Exit! GC5035S_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC5035S_AGC_Param[gain_index][1], temp_gain, reg_gain);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC5035S_MIRROR);
	...
	write_cmos_sensor(0x54, GC5035S_RSTDUMMY1);
	...
	write_cmos_sensor(0x22, GC5035S_RSTDUMMY2);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc5035s_otp_identify();
				pr_debug("[gc5035s_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
...
UINT32 GC5035SMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	cam_pr_debug("E\n");
