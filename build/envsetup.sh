...
function lunch()
{
    ...
    export TARGET_BUILD_TYPE=release
	#add by zhangshasha 20211209 begin
	TARGET_PRODUCT_NAME=${TARGET_PRODUCT#*_}
    echo "TARGET_PRODUCT_NAME=$TARGET_PRODUCT_NAME"
    source device/common/tool/hand_lk_config $TARGET_PRODUCT_NAME
    #add by zhangshasha 20211209 end
    ...
    destroy_build_var_cache
#Agenew add by liubao for match materiel @20211211{
    rm device/common/tool/compatible_mark.check_result
    if [ -f device/common/tool/compatible_mark.check_result ]; then
        echo "ERROR: cannot remove agenew/tool/compatible_mark.check_result, pls remove it!"
        exit
    fi
     
    source device/common/tool/sh/check_compatible_mark
    # @}
    source device/common/tool/handle_BROM_name $TARGET_PRODUCT_NAME
}
...
function _wrap_build()
{
    ...
    echo " ####${color_reset}"
    #added by zhangzhaopei to generate db file begin 20200117
    if [ ! -f device/common/tool/autocopy_ModemDB ]; then
		echo "ERROR: autocopy_ModemDB not exist,Unable to generate db file"
	else
		source device/common/tool/autocopy_ModemDB $TARGET_PRODUCT_NAME 
	fi
    #added by zhangzhaopei to generate db file begin 20200117
    echo
    return $ret
