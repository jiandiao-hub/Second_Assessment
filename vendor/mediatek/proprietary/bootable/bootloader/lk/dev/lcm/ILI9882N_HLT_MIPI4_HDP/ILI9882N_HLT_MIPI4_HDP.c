...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dsi.vertical_sync_active				= 10;//8   4
    params->dsi.vertical_backporch					= 32;//20;//12  10
    params->dsi.vertical_frontporch					= 380;//28;//2  18
    params->dsi.vertical_active_line				= FRAME_HEIGHT;

    params->dsi.horizontal_sync_active				= 10;//8
    params->dsi.horizontal_backporch				= 30;//40;//30
    params->dsi.horizontal_frontporch				= 40;//40;//30
    params->dsi.horizontal_active_pixel			= FRAME_WIDTH;

    params->dsi.PLL_CLOCK = 405;//385//196;//160
...
static kal_uint32 lcd_read_byte (kal_uint8 addr, kal_uint8 *dataBuffer)
{
	kal_uint32 ret_code = I2C_OK;
	kal_uint16 len;
	*dataBuffer = addr;
	...
	ret_code = i2c_write_read(&lcd_i2c, dataBuffer, len, len);
...
{
	Lcd_Log("ILI9882N_lcm %s %d \n",__FUNCTION__,__LINE__);

	mt_set_gpio_mode(164, 0);
	mt_set_gpio_dir(164, 1);
	mt_set_gpio_out(164, 1);

	mt_set_gpio_mode(165, 0);
	mt_set_gpio_dir(165,1);
	mt_set_gpio_out(165, 1);
...
static unsigned int lcm_compare_id(void)
{
	...
	id = (id_high << 8) | id_low;
	Lcd_Log("[ %s %d] 9882 id:0x%08x \n",__FUNCTION__,__LINE__, id);
#ifdef LCM_ATA_CHECK
	lcm_ata_check_id = LCM_ID;
#endif//LCM_ATA_CHECK
	if(id == LCM_ID)
	{
		Lcd_Log("[ %s %d] 9882 id:0x%08x \n",__FUNCTION__,__LINE__, id);
...
LCM_DRIVER ILI9882N_HLT_MIPI4_HDP_lcm_drv =
{
    .name			= "ILI9882N_HLT_MIPI4_HDP",
