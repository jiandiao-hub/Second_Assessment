...
#ifdef LCM_ATA_CHECK
unsigned int lcm_ata_check_id=0;
#endif
...
#define META_INIT_RC        "/system_ext/etc/init/hw/meta_init.system.rc"
		   ...
		   void *ramdisk, unsigned ramdisk_sz)
{
	...
	extern int update_mmc_status(void *fdt)__attribute__((weak));
	if (update_mmc_status)
		update_mmc_status(fdt);
	else
		pal_log_err("NOT Support update_mmc_status\n");
...
#endif
	{
       ...
       snprintf(tmpbuf, TMPBUF_SIZE, "lcm=%1d-%s", DISP_IsLcmFound(), mt_disp_get_lcm_id());
       cmdline_append(tmpbuf);
	...
	if (eBuildType == BUILD_TYPE_ENG) {
		...
		cmdline_append("panic_on_taint=20");
	}
	...
	/* bootconfig parses the g_cmdline,
	 * so have_bootconfig() needs to execute after append all cmdline in g_cmdline.
	 */
	bootconfig_sz = have_bootconfig((unsigned int)ramdisk + ramdisk_sz, cmdline_get());
	ret = fdt_setprop_cell(fdt, offset, "linux,initrd-end",
			       (unsigned int)ramdisk + ramdisk_sz + bootconfig_sz);
	if (ret) {
		assert(0);
		return FALSE;
	}
	...
	case BOOT_HEADER_VERSION_FOUR:
		ret = fdt_setprop_string(fdt, offset, "bootargs", (char *)non_androidboot_cmdline_get());
		break;
...
#ifdef LCM_ATA_CHECK
	snprintf(tmpbuf, TMPBUF_SIZE, "lcm_id=%x", lcm_ata_check_id);
