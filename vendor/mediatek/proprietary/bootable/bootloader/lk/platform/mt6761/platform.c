...
//wqm add
#ifdef AGN_LK_BOOT_VIBRATE_MDELAY
	extern void vibr_Enable_HW(void);
	extern void vibr_Disable_HW(void);
	if ((AGN_LK_BOOT_VIBRATE_MDELAY <= 25) || (AGN_LK_BOOT_VIBRATE_MDELAY >= 250)) 
		video_printf(" => [FAQ13305] [Vibrator] AGN_LK_BOOT_VIBRATE_MDELAY=%d\n", AGN_LK_BOOT_VIBRATE_MDELAY);
	vibr_Enable_HW();//Turn on the vibrator
	mdelay(AGN_LK_BOOT_VIBRATE_MDELAY); //Vibrate 100ms
	vibr_Disable_HW();//Turn off the vibrator
#endif
//
