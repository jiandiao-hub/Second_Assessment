...
void mt_disp_show_battery_capacity(UINT32 capacity)
{
	...
	if (logo_cust_if->show_battery_capacity) {
		...
		fill_animation_battery_by_ver(capacity, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen, show_animationm_ver);
...
void mt_disp_show_charger_ov_logo(void)
{
	...
	if (logo_cust_if->show_boot_logo) {
		...
		init_fb_screen();
		fill_animation_logo(CHARGER_OV_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
		mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
	}
...
void mt_disp_show_charging(int index)
{
	...
	if (logo_cust_if->show_boot_logo) {
		...
		init_fb_screen();
		//show_logo(2);
		fill_animation_logo(index + LOW_BATTERY01_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
		mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
	}
...
void mt_disp_show_plug_charger(void)
{
	...
	if (logo_cust_if->show_boot_logo) {
		...
		init_fb_screen();
		//show_logo(2);
		fill_animation_logo(LOW_BATTERY_REMIND_INDEX, mt_get_fb_buf(), (void *)mt_get_tempfb_addr(), logo_addr, phical_screen);
		mt_disp_update(0, 0, CFG_DISPLAY_WIDTH, CFG_DISPLAY_HEIGHT);
	}
