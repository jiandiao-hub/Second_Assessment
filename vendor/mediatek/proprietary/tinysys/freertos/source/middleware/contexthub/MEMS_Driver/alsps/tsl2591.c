...
static struct sampleAlsTask *sampleAlsDebugPoint;
...
static int sampleAls_read_status(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
...
static int sampleAls_read_raw(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                            void *inBuf, uint8_t inSize, uint8_t elemInSize,
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_get_enable(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                            void *inBuf, uint8_t inSize, uint8_t elemInSize,
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_set_gain(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                            void *inBuf, uint8_t inSize, uint8_t elemInSize,
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_set_als_dis(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                            void *inBuf, uint8_t inSize, uint8_t elemInSize,
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    mTask.txBuf[0] = TSL2591_ENABLE_REG;
...
static int sampleAls_set_als_en(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                            void *inBuf, uint8_t inSize, uint8_t elemInSize,
                            void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    mTask.txBuf[0] = TSL2591_ENABLE_REG;
    mTask.chip.als_enable &= ~AEN;
...
static int sampleAls_get_als_lux(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                 void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                 void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static void alsGetSensorInfo(struct sensorInfo_t *data) {
    strncpy(data->name, "tsl2591", sizeof(data->name));
}

static int sampleAls_set_als_enable(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                       void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                       void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    mTask.txBuf[0] = TSL2591_ENABLE_REG;
...
static int sampleAls_set_als_power_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_set_als_power_delay(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_set_als_power_off(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                     void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                     void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_als_ratechg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                               void *inBuf, uint8_t inSize, uint8_t elemInSize,
                               void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    return 0;
...
static int sampleAls_als_cali(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    int32_t alsCali[2];
    float   alsCali_mi;
...
static int sampleAls_als_cfg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    int ret = 0;
...
static int sampleAls_set_als_control(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    mTask.txBuf[0] = TSL2591_CONTROL_REG;
...
static int sampleAls_set_als_persist(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int sampleAls_register_core(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                 void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                 void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static struct sensorFsm tsl2591AlsFsm[] = {
    sensorFsmCmd(STATE_SAMPLE_ALS, STATE_GET_ALS_VAL, sampleAls_read_status),
    sensorFsmCmd(STATE_GET_ALS_VAL, STATE_GET_ALS_EN, sampleAls_read_raw),
    sensorFsmCmd(STATE_GET_ALS_EN, STATE_SET_ALS_GAIN, sampleAls_get_enable),
    sensorFsmCmd(STATE_SET_ALS_GAIN, STATE_SET_ALS_DIS, sampleAls_set_gain),
    sensorFsmCmd(STATE_SET_ALS_DIS, STATE_SET_ALS_EN, sampleAls_set_als_dis),
    sensorFsmCmd(STATE_SET_ALS_EN, STATE_GET_ALS_LUX, sampleAls_set_als_en),
    sensorFsmCmd(STATE_GET_ALS_LUX, STATE_SAMPLE_ALS_DONE, sampleAls_get_als_lux),
    /* als enable state */
    sensorFsmCmd(STATE_ALS_ENABLE, STATE_ALS_POWER_DELAY, sampleAls_set_als_power_on),
    sensorFsmCmd(STATE_ALS_POWER_DELAY, STATE_SET_ALS_EN, sampleAls_set_als_power_delay),
    sensorFsmCmd(STATE_SET_ALS_EN, STATE_ALS_ENABLE_DONE, sampleAls_set_als_enable),
    /* als disable state */
    sensorFsmCmd(STATE_ALS_DISABLE, STATE_ALS_DISABLE_DONE, sampleAls_set_als_power_off),
    /* als rate state */
    sensorFsmCmd(STATE_ALS_RATECHG, CHIP_ALS_RATECHG_DONE, sampleAls_als_ratechg),
    /* als cali state */
    sensorFsmCmd(STATE_ALS_CALI, CHIP_ALS_CALI_DONE, sampleAls_als_cali),
    /* als cfg state */
    sensorFsmCmd(STATE_ALS_CFG, CHIP_ALS_CFG_DONE, sampleAls_als_cfg),
    /* init state */
    sensorFsmCmd(STATE_ALS_RESET, STATE_ALS_PST, sampleAls_set_als_control),
    sensorFsmCmd(STATE_ALS_PST, STATE_CORE, sampleAls_set_als_persist),
    sensorFsmCmd(STATE_CORE, CHIP_PS_RESET, sampleAls_register_core),
};

static int sampleAlsInit(void) {
    int ret = 0, retry;

    osLog(LOG_INFO, "%s: task starting\n", __func__);
    sampleAlsDebugPoint = &mTask;
...
MODULE_DECLARE(tsl2591, SENS_TYPE_ALS, sampleAlsInit);
#else
#include "mtk_overlay_init.h"
OVERLAY_DECLARE(tsl2591, OVERLAY_ID_ALSPS, sampleAlsInit);
#endif
