...
void baroSendCalibrationResult(uint8_t sensorType, int32_t value[2]) {
    struct CalibrationData *data = heapAlloc(sizeof(struct CalibrationData));
    if (!data) {
        osLog(LOG_ERROR, "Couldn't alloc data cali result pkt");
        return;
    }
    data->header.appId = APP_ID_MAKE(APP_ID_VENDOR_MTK, MTK_APP_ID_WRAP(SENS_TYPE_BARO, 0, 0));
...
static bool sensorCalibrateBaro(void *cookie)
{
    ...
    if (mTask.nowState == CHIP_IDLE) {
        ...
        if (sensorFsmRunState(&dataInfo, &mTask.fsm, (const void *)CHIP_CALIBRATE, &i2cCallback, &spiCallback) < 0)
            mTask.nowState = CHIP_IDLE;
    } else {
        osLog(LOG_INFO, "sensorCaliBaro pending by:%d\n", mTask.nowState);
        mTask.pendingCali = true;
    }
    return true;
}

static void setBaroHwCaliCfg(int32_t *data)
{
    ...
    mTask.nowState = CHIP_CFGCALI;
    dataInfo.inBuf = (void *)&caliCfgPacket;
    dataInfo.inSize = 1;
    dataInfo.elemInSize = sizeof(struct baroCaliCfgPacket);
    dataInfo.outBuf = NULL;
    dataInfo.outSize = 0;
    dataInfo.elemOutSize = 0;
    if (sensorFsmRunState(&dataInfo, &mTask.fsm, (const void *)CHIP_CFGCALI, &i2cCallback, &spiCallback) < 0)
        mTask.nowState = CHIP_IDLE;
}
...
static bool sensorCfgDataBaro(void *data, void *cookie)
{
    ...
    osLog(LOG_INFO, "%s [ref, offset]: [%d, %d]\n", __func__, values[0], values[1]);
    if (mTask.nowState == CHIP_IDLE) {
        setBaroHwCaliCfg(values);
    } else {
        osLog(LOG_INFO, "sensorCfgDataBaro pending by:%d\n", mTask.nowState);
        mTask.pendingCfgCali = true;
        mTask.pendCfgData[0] = values[0];
        mTask.pendCfgData[1] = values[1];
    }
    return true;
}
...
{
    if (mTask.pendingInt && mTask.baroNowOn) {
        mTask.pendingInt = false;
        osEnqueuePrivateEvt(EVT_SENSOR_INTR, NULL, NULL, mTask.id);
        return;
    }
    if (mTask.pendingSample && mTask.baroNowOn) {
    ...
    if (mTask.pendingCali) {
        ...
        sensorOpsBaro.sensorCalibrate(NULL);
        return;
    }
    if (mTask.pendingCfgCali) {
        mTask.pendingCfgCali = false;
        sensorOpsBaro.sensorCfgData(mTask.pendCfgData, NULL);
        return;
    }
}
...
static void handleSensorEvent(const void *state)
{
    ...
    switch (mState) {
        ...
        case CHIP_CALIBRATE_DONE: {
            sensorFsmReleaseState(&mTask.fsm);
            mTask.nowState = CHIP_IDLE;
            osLog(LOG_INFO, "baro: calibrate done\n");
            processPendingEvt();
            break;
        }

        case CHIP_CFGCALI_DONE: {
            sensorFsmReleaseState(&mTask.fsm);
            mTask.nowState = CHIP_IDLE;
            osLog(LOG_INFO, "baro: cfgcali done\n");
            processPendingEvt();
            break;
        }
