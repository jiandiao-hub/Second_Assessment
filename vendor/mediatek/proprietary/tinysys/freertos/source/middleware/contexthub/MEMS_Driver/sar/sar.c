...
#if 0
static uint32_t sar_test_timer_handle = 0;
SRAM_REGION_FUNCTION static void SarTestTimerCallback(uint32_t timerId, void *cookie)
{
    ENTER;
    if(mTask.intrMode == SAR_POLLING_MODE){
        /* PRINT_INF("SarTestTimerCallback:cookie:%d\n", (uint32_t)cookie); */
        osEnqueuePrivateEvt(EVT_SENSOR_TIMER, cookie, NULL, mTask.id);
    }
}
#endif
...
static void handleSensorEvent(const void *state)
{
    ...
    switch (mState) {
        ...
        if (mTask.fsm.mSensorFsm != NULL) {
            PRINT_INF("sar: enable sar for test\n");
            struct transferDataInfo dataInfo;
            dataInfo.inBuf = NULL;
            dataInfo.inSize = 0;
            dataInfo.elemInSize = 0;
            ...
            dataInfo.outSize = NULL;
            dataInfo.elemOutSize = NULL;
            sensorFsmRunState(&dataInfo, &mTask.fsm, (const void *)CHIP_SAR_ENABLE, &i2cCallback, &spiCallback);
        } else
            PRINT_ERR("sar: can NOT enable sar for test\n");


        
        if(mTask.intrMode == SAR_POLLING_MODE){
            uint64_t sar_test_timer_delay;
            sar_test_timer_delay = 1000000000ull/2;     //1s  1000000000ull/2=500ms
            PRINT_INF("start test timer. interval=%ulns\n", sar_test_timer_delay);
            
            if (sar_test_timer_handle)
                timTimerCancel(sar_test_timer_handle);

               
            sar_test_timer_handle = timTimerSet(sar_test_timer_delay, 0, 50, SarTestTimerCallback, NULL, false);
            if (sar_test_timer_handle == 0)
                configASSERT(0);
        }
#endif
            processPendingEvt();
        ...
        case CHIP_SAR_ENABLE_DONE: {
            ...
            mTask.nowState = CHIP_IDLE;
            osLog(LOG_INFO, "sar: xxx enable done\n");
            processPendingEvt();
            break;
        }
        ...
        case CHIP_SAR_RATECHG_DONE: {
            sensorFsmReleaseState(&mTask.fsm);
	    mTask.intrMode = SAR_POLLING_MODE;
             if (mTask.intrMode == SAR_POLLING_MODE) {
		 ...
		 osLog(LOG_INFO, "sar CHIP_SAR_RATECHG_DONE :%dns\n", timerDelay);
        ...
        case CHIP_SAR_SAMPLING_DONE: {
            sensorFsmReleaseState(&mTask.fsm);
            osLog(LOG_INFO, "CHIP_SAR_SAMPLING_DONE ENTER\n");
            ...
            processPendingEvt();
            osLog(LOG_INFO, "CHIP_SAR_SAMPLING_DONE HHHH\n");
            break;
        }
    }
}
...
{
    struct transferDataInfo dataInfo;
    osLog(LOG_ERROR, "sar evtType :0x%08x evtdata:%d\n", evtType, (enum SarFsmState)evtData);

    switch (evtType) {
 //xuyanan
        osLog(LOG_INFO, "sar: handleEvent\n");
        case EVT_APP_START: {
            if (mTask.fsm.mSensorFsm != NULL) {
                ...
                dataInfo.outSize = NULL;
                dataInfo.elemOutSize = NULL;
            	//handleSensorEvent((const void *)(CHIP_INIT_DONE));
...
//xuyanan
          osLog(LOG_INFO, "sar: EVT_SENSOR_EVENT\n");
            break;
        }

        case EVT_SENSOR_TIMER: {
            /* Start sampling for a value */
			osLog(LOG_INFO, "Start sampling for a value\n");
            if (mTask.nowState == CHIP_IDLE) {
                if (!mTask.mSensorPerUnit.isReading) {
					osLog(LOG_INFO, "EVT_SENSOR_TIMER\n");
...
static bool startTask(uint32_t taskId)
{
    ...
    /* Register tid first */
    mTask.id = taskId;    
    mTask.intrMode = SAR_WORK_MODE; //yorke
