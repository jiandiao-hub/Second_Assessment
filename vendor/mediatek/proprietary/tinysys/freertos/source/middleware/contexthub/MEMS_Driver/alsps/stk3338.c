...
static struct stk3338Task *stk3338DebugPoint;
...
static int stk3338_read_als_data(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
	...
	osLog(LOG_ERROR, "stk3338_read_als_data...\n");  
...
static int stk3338_get_als_value(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
	...
	osLog(LOG_ERROR, "stk3338_get_als_value: als=%d, lux=%d\n", mTask.als_raw_data, mTask.data[0].als_data); 
    } 
...
static int stk3338_read_ps(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                          void *inBuf, uint8_t inSize, uint8_t elemInSize,
                          void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(mTask.psPowerOn == false){
        osLog(LOG_ERROR, "stk3332sv_read_ps power off"); 
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
        return 0;        
    }
    if (rxTransferDataInfo(&mTask.dataInfo, inBuf, inSize, elemInSize, outBuf, outSize, elemOutSize)) {
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, ERROR_EVT);
        osLog(LOG_ERROR, "stk3338 ps, rx dataInfo error\n");
        return -1;
    }
	osLog(LOG_ERROR, "stk3338_read_ps...\n"); 
...
static int stk3338_read_ps_flag(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                          void *inBuf, uint8_t inSize, uint8_t elemInSize,
                          void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(mTask.psPowerOn == false){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_get_ps_status(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    //uint32_t    ps_threshold_high;
    //uint32_t    ps_threshold_low;
...
static int stk3338_get_ps_raw_data(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(mTask.psPowerOn == false){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_clr_int(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(mTask.psPowerOn == false){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    ...
    osLog(LOG_ERROR, "stk3338_clr_int...\n");                              
    mTask.txBuf[0] = STK_FLAG_REG;
...
static int stk3338_enable_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(mTask.psPowerOn == false){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    ...
    if(mTask.ps_int_flag == true){                                   
		 osLog(LOG_ERROR, "stk3338_enable_eint...\n");    
...
static bool stk3338_is_skip(void) {
    if((mTask.psPowerOn == false)
...
static void psGetData(void *sample) {
    ...
    void get_ps_data(void *cookie, size_t tx, size_t rx, int err) {
        ...
        if (err == 0) {
            ...
            osLog(LOG_INFO, "stk3338: read ps data i2c error (%d)\n", err);
    }
...
static int stk3338_ps_set_threshold(uint32_t threshold_high, uint32_t threshold_low) {
    char txBuf[6];
...
static void psSetCalibration(int32_t *cali, int32_t size) {
    mTask.psCali = cali[0];
    stk3338_ps_set_threshold(mTask.ps_threshold_high + mTask.psCali,
...
static void psSetThreshold(uint32_t threshold_high, uint32_t threshold_low) {
    ...
    stk3338_ps_set_threshold(mTask.ps_threshold_high, mTask.ps_threshold_low);
}

static void psGetSensorInfo(struct sensorInfo_t *data)
{
    strncpy(data->name, "stk3338_p", sizeof(data->name));
}

static void alsGetSensorInfo(struct sensorInfo_t *data)
{
    strncpy(data->name, "stk3338_l", sizeof(data->name));
}

static int stk3338_mask_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                         void *inBuf, uint8_t inSize, uint8_t elemInSize,
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_unmask_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                         void *inBuf, uint8_t inSize, uint8_t elemInSize,
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    mt_eint_unmask(mTask.hw->eint_num);
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_set_als_debounce_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                      void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                      void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_INFO, "stk3338_set_als_debounce_on\n");
...
static int stk3338_set_ps_debounce_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                     void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                     void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_INFO, "stk3338_set_ps_debounce_on\n");
...
static int stk3338_set_als_power_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_als_power_off(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_als_ratechg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_als_cali(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    //int32_t alsCali[2];
    //float   alsCali_mi;
...
static int stk3338_als_cfg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    int ret = 0;
    ...
    if (ret < 0) {
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
        osLog(LOG_ERROR, "stk3338_als_cfg, rx inSize and elemSize error\n");
        return -1;
    }
    //alsCali_mi = (double)((float)caliCfgPacket.caliCfgData[0] / ALS_INCREASE_NUM_AP);
    //APS_ALOGD("stk3338_als_cfg: [%f]\n", alsCali_mi);
    mTask.alsCali = caliCfgPacket.caliCfgData[0];
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    return 0;
}

static int stk3338_set_ps_power_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_ps_power_on_set_int(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_power_on_get_ps_thd(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                          void *inBuf, uint8_t inSize, uint8_t elemInSize,
                          void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_power_on_set_ps_thd(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_compare_sun_level()
{
...
static int stk3338_get_flag_pre(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF(); 
...
static int stk3338_get_flag(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
	int ps_flag;
...
static int stk3338_get_ps_rawdata_pre(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF(); 
...
static int stk3338_get_ps_rawdata(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(stk3338_is_skip()){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_get_psoff_pre(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF(); 
...
static int stk3338_get_psoff(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(stk3338_is_skip()){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_get_threshold_pre(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF(); 
...
static int stk3338_get_threshold(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_get_allreg_pre(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF(); 
...
static int stk3338_get_allreg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    if(stk3338_is_skip()){
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3338_ps_tune_start(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
...
static int stk3338_ps_tune_fae(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) 
{
    //APS_ALOGF();
	...
	if(stk3338_compare_sun_level() != 0){
...
void psTimerCallback(uint32_t timerId, void *cookie) {
    //alsPsInterruptOccur();
	stk3338_prx_tune_zero_func_fae();
}
#endif
static int stk3338_set_ps_power_off(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_ps_power_off_clr_int(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_power_off_reset_thd(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_ps_ratechg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_ps_cali(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
//    int32_t threshold[2];
...
static int stk3338_ps_cfg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    int ret = 0;
    struct alspsCaliCfgPacket caliCfgPacket;
    ...
    if (ret < 0) {
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
        osLog(LOG_ERROR, "stk3338_ps_cfg, rx inSize and elemSize error\n");
        return -1;
    }
	...
	if(caliCfgPacket.caliCfgData[0] != 0){
    	...
    	//stk3338_ps_set_threshold(caliCfgPacket.caliCfgData[0], caliCfgPacket.caliCfgData[1]);
	}
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    return 0;
}

static int stk3338_get_alsps_state(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                               void *inBuf, uint8_t inSize, uint8_t elemInSize,
                               void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_wait_time(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_sw_reset(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_sw_reset_wait(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_alsps_ctrl(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_alsctrl2(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_alsctrl2...");                          
...
static int stk3338_set_intelli_wait(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_intelli_wait...");                          
...
static int stk3338_set_bgir(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_bgir...");                          
...
static int stk3338_set_pd(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_pd...");                          
    mTask.txBuf[0] = STK_PD_REG;
    mTask.txBuf[1] = 0x0F; //ps0+1+2+3
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 2,
                       i2cCallBack, next_state);
}

static int stk3338_set_again(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_again...");                          
...
static int stk3338_set_spec1(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_spec1...");                          
...
static int stk3338_set_irdr(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_ERROR, "stk3338_set_irdr...");                          
    mTask.txBuf[0] = 0xF4;
    mTask.txBuf[1] = 0x00;
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 2,
                       i2cCallBack, next_state);
...
static int stk3338_set_ps_thdl(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static int stk3338_set_ps_thdh(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static void stk3338_eint_handler(int arg) {
    osLog(LOG_INFO, "stk eint handler\n");
    alsPsInterruptOccur();
}

static int stk3338_setup_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
    osLog(LOG_INFO, "stk setup eint\n");                           
    mt_eint_dis_hw_debounce(mTask.hw->eint_num);
    mt_eint_registration(mTask.hw->eint_num, LEVEL_SENSITIVE, LOW_LEVEL_TRIGGER, stk3338_eint_handler, EINT_INT_UNMASK,
                         EINT_INT_AUTO_UNMASK_OFF);
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    return 0;
}

static int stk3338_register_core(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize) {
...
static struct sensorFsm stk3338Fsm[] = {
    /* sample als */
    sensorFsmCmd(STATE_SAMPLE_ALS, STATE_GET_ALS_DATA, stk3338_read_als_data),
    sensorFsmCmd(STATE_GET_ALS_DATA, STATE_SAMPLE_ALS_DONE, stk3338_get_als_value),

    /* sample ps */
	sensorFsmCmd(STATE_SAMPLE_PS, STATE_GET_PS_FLG_RAW, stk3338_read_ps),
    sensorFsmCmd(STATE_GET_PS_FLG_RAW, STATE_GET_PS_FLG, stk3338_read_ps_flag),
    sensorFsmCmd(STATE_GET_PS_FLG, STATE_GET_PS_RAW_DATA, stk3338_get_ps_status),
    sensorFsmCmd(STATE_GET_PS_RAW_DATA, STATE_PS_TUNE_FAE, stk3338_get_ps_raw_data),

    /*ps tune state*/
    sensorFsmCmd(STATE_PS_TUNE_FAE, STATE_PS_TUNE_FAE_STEP1, stk3338_ps_tune_start),  
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP1, STATE_PS_TUNE_FAE_STEP2, stk3338_get_psoff_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP2, STATE_PS_TUNE_FAE_STEP3, stk3338_get_psoff),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP3, STATE_PS_TUNE_FAE_STEP4, stk3338_get_ps_rawdata_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP4, STATE_PS_TUNE_FAE_STEP5, stk3338_get_ps_rawdata),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP5, STATE_PS_TUNE_FAE_STEP6, stk3338_get_threshold_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP6, STATE_PS_TUNE_FAE_STEP7, stk3338_get_threshold),  
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP7, STATE_PS_TUNE_FAE_STEP8, stk3338_get_flag_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP8, STATE_PS_TUNE_FAE_STEP9, stk3338_get_flag),  
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP9, STATE_PS_TUNE_FAE_STEP10, stk3338_get_allreg_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP10, STATE_PS_TUNE_FAE_STEP11, stk3338_get_allreg),  
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP11, STATE_PS_TUNE_FAE_STEP12, stk3338_get_threshold_pre),
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP12, STATE_PS_TUNE_FAE_STEP13, stk3338_get_threshold),   
    sensorFsmCmd(STATE_PS_TUNE_FAE_STEP13, STATE_CLR_INT, stk3338_ps_tune_fae),

    sensorFsmCmd(STATE_CLR_INT, STATE_EN_EINT, stk3338_clr_int),
    sensorFsmCmd(STATE_EN_EINT, STATE_SAMPLE_PS_DONE, stk3338_enable_eint),

    /* als enable state */
    sensorFsmCmd(STATE_ALS_ENABLE, STATE_ALS_POWER_ON, stk3338_get_alsps_state),
    sensorFsmCmd(STATE_ALS_POWER_ON, STATE_ALS_SET_DEBOUNCE, stk3338_set_als_power_on),
    sensorFsmCmd(STATE_ALS_SET_DEBOUNCE, STATE_ALS_ENABLE_DONE, stk3338_set_als_debounce_on),

    /* als disable state */
    sensorFsmCmd(STATE_ALS_DISABLE, STATE_ALS_POWER_OFF, stk3338_get_alsps_state),
    sensorFsmCmd(STATE_ALS_POWER_OFF, STATE_ALS_DISABLE_DONE, stk3338_set_als_power_off),
    /* als cali state */
    sensorFsmCmd(STATE_ALS_CALI, CHIP_ALS_CALI_DONE, stk3338_als_cali),
    /* als cfg state */
    sensorFsmCmd(STATE_ALS_CFG, CHIP_ALS_CFG_DONE, stk3338_als_cfg),

    /* ps enable state */
    sensorFsmCmd(STATE_PS_ENABLE, STATE_PS_POWER_ON, stk3338_get_alsps_state),
    sensorFsmCmd(STATE_PS_POWER_ON, STATE_PS_POWER_ON_SET_INT, stk3338_set_ps_power_on),
    sensorFsmCmd(STATE_PS_POWER_ON_SET_INT, STATE_PS_POWER_ON_GET_PS_THD, stk3338_set_ps_power_on_set_int),
    sensorFsmCmd(STATE_PS_POWER_ON_GET_PS_THD, STATE_PS_POWER_ON_SET_PS_THD, stk3338_power_on_get_ps_thd),
    sensorFsmCmd(STATE_PS_POWER_ON_SET_PS_THD, STATE_PS_SET_DEBOUNCE, stk3338_power_on_set_ps_thd),
    sensorFsmCmd(STATE_PS_SET_DEBOUNCE, STATE_PS_UNMASK_EINT, stk3338_set_ps_debounce_on),
    sensorFsmCmd(STATE_PS_UNMASK_EINT, STATE_PS_ENABLE_DONE, stk3338_unmask_eint),
    /* ps disable state */
    sensorFsmCmd(STATE_PS_DISABLE, STATE_GET_ALSPS_STATE, stk3338_mask_eint),
    sensorFsmCmd(STATE_GET_ALSPS_STATE, STATE_PS_POWER_OFF, stk3338_get_alsps_state),
    sensorFsmCmd(STATE_PS_POWER_OFF, STATE_PS_POWER_OFF_CLR_INT, stk3338_set_ps_power_off),
    sensorFsmCmd(STATE_PS_POWER_OFF_CLR_INT, STATE_PS_POWER_OFF_RESET_THD, stk3338_set_ps_power_off_clr_int),
    sensorFsmCmd(STATE_PS_POWER_OFF_RESET_THD, STATE_PS_DISABLE_DONE, stk3338_power_off_reset_thd),
    /* als rate change */
    sensorFsmCmd(STATE_ALS_RATECHG, STATE_ALS_RATECHG_DONE, stk3338_als_ratechg),
    /* ps rate change */
    sensorFsmCmd(STATE_PS_RATECHG, STATE_PS_RATECHG_DONE, stk3338_ps_ratechg),

    /* ps cali state */
    sensorFsmCmd(STATE_PS_CALI, CHIP_PS_CALI_DONE, stk3338_ps_cali),
    /* ps cfg state */
    sensorFsmCmd(STATE_PS_CFG, CHIP_PS_CFG_DONE, stk3338_ps_cfg),

    /* init state */
    sensorFsmCmd(STATE_RESET, STATE_SET_SW_RST, stk3338_set_wait_time),
    sensorFsmCmd(STATE_SET_SW_RST, STATE_SET_SW_RST_WAIT02, stk3338_set_sw_reset),
    sensorFsmCmd(STATE_SET_SW_RST_WAIT02, STATE_SET_SW_RST_WAIT03, stk3338_sw_reset_wait),
    sensorFsmCmd(STATE_SET_SW_RST_WAIT03, STATE_SET_ALSPS_CTRL, stk3338_sw_reset_wait),
    sensorFsmCmd(STATE_SET_ALSPS_CTRL, STATE_SET_ALSCTRL2, stk3338_set_alsps_ctrl),
    sensorFsmCmd(STATE_SET_ALSCTRL2, STATE_SET_INTELLI_WAIT, stk3338_set_alsctrl2),
    sensorFsmCmd(STATE_SET_INTELLI_WAIT, STATE_SET_BGIR, stk3338_set_intelli_wait),
    sensorFsmCmd(STATE_SET_BGIR, STATE_SET_PD, stk3338_set_bgir),
    sensorFsmCmd(STATE_SET_PD, STATE_SET_AGAIN, stk3338_set_pd),
    sensorFsmCmd(STATE_SET_AGAIN, STATE_SET_SPEC1, stk3338_set_again),
    sensorFsmCmd(STATE_SET_SPEC1, STATE_SET_IRDR, stk3338_set_spec1),
    sensorFsmCmd(STATE_SET_IRDR, STATE_SET_PS_THDH, stk3338_set_irdr),
    sensorFsmCmd(STATE_SET_PS_THDH, STATE_SET_PS_THDL, stk3338_set_ps_thdh),
    sensorFsmCmd(STATE_SET_PS_THDL, STATE_SETUP_EINT, stk3338_set_ps_thdl),
    sensorFsmCmd(STATE_SETUP_EINT, STATE_CORE, stk3338_setup_eint),
    sensorFsmCmd(STATE_CORE, STATE_INIT_DONE, stk3338_register_core),
};

static int stk3338Init(void) {
    int ret = 0;
    osLog(LOG_INFO, "stk init\n");
    stk3338DebugPoint = &mTask;

    mTask.hw = get_cust_alsps("stk3338");
    if (NULL == mTask.hw) {
        osLog(LOG_ERROR, "get_cust_acc_hw fail\n");
        return -1;
    }
    mTask.i2c_addr = mTask.hw->i2c_addr[0]; // Proximity Sensor STK3338
    ...
    mTask.ledctrl_val = STK_LED_25MA;
    ...
    osLog(LOG_INFO, "stk3338 i2c_num=%d, i2d_addr=%02x\n", mTask.hw->i2c_num, mTask.i2c_addr);
    ...
    for (uint8_t i = 0; i < 3; i++) {
		...
		mdelay(10);
		osLog(LOG_ERROR, "stk3338 i2cMasterTxRxSync fail i=%d\n", i);
    }
	
    if (ret < 0) {
        osLog(LOG_ERROR, "stk3338 i2cMasterTxRxSync fail!!!\n");
        ret = -1;
        i2cMasterRelease(mTask.hw->i2c_num);
        goto err_out;
    }

    osLog(LOG_INFO, "stk3338 deviceId: 0x%x\n", mTask.deviceId);
		
    alsSensorRegister();
    psSensorRegister();
    registerAlsPsDriverFsm(stk3338Fsm, ARRAY_SIZE(stk3338Fsm));
err_out:
    return ret;
}

#ifndef CFG_OVERLAY_INIT_SUPPORT
MODULE_DECLARE(stk3338, SENS_TYPE_ALS, stk3338Init);
#else
#include "mtk_overlay_init.h"
OVERLAY_DECLARE(stk3338, OVERLAY_WORK_02, stk3338Init);
#endif
