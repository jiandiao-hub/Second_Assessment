...
enum BaroTempState {
    ...
    CHIP_RATECHG_DONE = 8,
    CHIP_CALIBRATE = 9,
    CHIP_CALIBRATE_DONE = 10,
    CHIP_CFGCALI = 11,
    CHIP_CFGCALI_DONE = 12,
    CHIP_INIT_DONE = 13,
    CHIP_IDLE = 14,
    CHIP_RESET = 15,
};
...
static inline int rxCaliCfgInfo(struct baroCaliCfgPacket *recvPacket, void *inBuf,
    uint8_t inSize, uint8_t elemInSize, void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    struct baroCaliCfgPacket *packet = (struct baroCaliCfgPacket *)inBuf;

    if (inBuf == NULL || inSize != 1 || elemInSize != sizeof(struct baroCaliCfgPacket)) {
        return -1;
    }
