...
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
	CFLAGS += -DCONFIG_MACH_MT6781
endif
...
CFG_MTK_VOW_MAX_PDK_NUMBER = 0
CFG_MTK_VOW_AMAZON_SUPPORT = no
CFG_MTK_VOW_GVA_SUPPORT = no
...
ifeq ($(CFG_MTK_ULTRASND_PROXIMITY),yes)
$(eval TOTAL_HEAP_SIZE=$(shell echo $$(($(TOTAL_HEAP_SIZE) + (56 * 1024)))))
endif
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
C_FILES  += $(TINYSYS_SECURE_DIR)/project/CM4_A/mt6781/platform/testsuite/src/ts_sample/src/i2c_testsuite.c
else
...
$(SOURCE_DIR)/drivers/$(PROCESSOR)/mt6768/pmic_wrap/inc
else
INCLUDES += $(SOURCE_DIR)/drivers/common/pmic_wrap/v2/inc \
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
INCLUDES += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6366
C_FILES  += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6366/vow_hw.c
LIBFLAGS += -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow/mt6785 -lblisrc_vow
else
INCLUDES += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6359
C_FILES  += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6359/vow_hw.c
endif

ifeq ($(CFG_MTK_VOW_GVA_SUPPORT),yes)
...
LIBFLAGS += -L$(THIRDPARTY_CM4_LIB_DIR)/google -lcortexm4_google_hotword_dsp_api -lm
...
ifeq ($(CFG_MTK_VOW_AMAZON_SUPPORT),yes)
...
ifeq ($(CFG_MTK_ULTRASND_PROXIMITY),yes)
  ifeq ($(CFG_MTK_VOW_BARGE_IN_SUPPORT),yes)
    CFLAGS += -DCFG_ULTRASOUND_AUDIO_SHARE_IRQ
  endif
endif
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
INCLUDES += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6366
C_FILES  += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6366/audio_hw.c
else
INCLUDES += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6359
C_FILES  += $(DRIVERS_PLATFORM_DIR)/audio/hardware/6359/audio_hw.c
endif
...
INCLUDES += $(SOURCE_DIR)/drivers/common/audio/share_irq/inc
...
C_FILES  += $(SOURCE_DIR)/drivers/common/audio/share_irq/src/audio_share_irq.c
...
ifeq ($(CFG_MTK_ULTRASND_PROXIMITY),yes)
  INCLUDES += $(SOURCE_DIR)/drivers/common/uSnd
  INCLUDES += $(SOURCE_DIR)/drivers/common/uSnd/interface
  INCLUDES += $(SOURCE_DIR)/drivers/common/audio/framework
  INCLUDES += $(SOURCE_DIR)/drivers/common/audio/hardware
  INCLUDES += $(SOURCE_DIR)/drivers/common/audio/tasks
  INCLUDES += $(SOURCE_DIR)/middleware/lib/aurisys/interface
...
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
  ...
  C_FILES  += $(SOURCE_DIR)/drivers/common/uSnd/ultrasound_driver.c
  C_FILES  += $(SOURCE_DIR)/drivers/common/uSnd/ultrasound_dma.c
  ifeq ($(strip $(CFG_ULTRASOUND_ENGINE)), DEMO)
    C_FILES  += $(SOURCE_DIR)/drivers/common/uSnd/demo/demo_ultrasound_engine.c
  endif
endif
...
# vow lib of MTK PDk
ifneq (,$(CFG_MTK_VOW_MAX_PDK_NUMBER))
ifneq (0,$(CFG_MTK_VOW_MAX_PDK_NUMBER))
INCLUDES += \
  $(SOURCE_DIR)/middleware/lib/audio/cm4/vow/inc
LIBFLAGS += \
  -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow -lvow -lm \
  -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow -lvow_dsputility -lm
endif
endif

ifeq ($(CFG_MTK_VOW_BARGE_IN_SUPPORT),yes)
ifeq ($(strip $(TARGET_BOARD_PLATFORM)), mt6781)
LIBFLAGS += -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow/mt6785 -lblisrc_accurate_vow_6781
else
LIBFLAGS += -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow/mt6785 -lblisrc_accurate_vow
endif
endif
...
FEATURE_LIBFLAGS := \
  -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow -lvow_dspinform
include $(REGISTER_FEATURE)
...
FEATURE_INCLUDES := \
  $(SOURCE_DIR)/middleware/lib/audio/cm4/vow/inc/api
FEATURE_LIBFLAGS := \
  -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow -laec \
  -L$(SOURCE_DIR)/middleware/lib/audio/cm4/vow -lsigStatus
include $(REGISTER_FEATURE)
