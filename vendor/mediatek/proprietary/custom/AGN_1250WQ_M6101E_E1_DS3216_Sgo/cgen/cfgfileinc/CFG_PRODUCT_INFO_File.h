...
typedef struct
{
    /*[default: 2, fail: 0, success: 1] */
    unsigned char       testitem_flags[64]; 
} s_mtk_meta_testitems;

/**
* RuntimeTest: Factory reset counters record
*/
typedef struct
{
    unsigned char factory_reset_left[4];        // unsigned int
    unsigned char factory_reset_completed[4];   // unsigned int
    unsigned char reserved[8];
} s_mtk_factory_reset_counters;
...
typedef struct
{
    ...
    s_mtk_meta_testitems        mtk_meta_testitems[3]; // 64B*3
    s_mtk_factory_reset_counters mtk_factory_reset_counters; // 16B
    unsigned char reserved[1024-170-64-40-64-3-64*3 - 16];
} PRODUCT_INFO;
