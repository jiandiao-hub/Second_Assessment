...
of_property_read_u32(pdev->dev.of_node,"config_vcn28_pin",&config_custom_param);
WMT_PLAT_PR_ERR("ant config_custom_param:%d\n", config_custom_param);
...
INT32 mtk_wcn_consys_hw_reg_ctrl(UINT32 on, UINT32 co_clock_type)
{
	...
	if (on) {
		...
		if (co_clock_type) {
			...
			{
			if(config_custom_param == 1)
			{
				wmt_consys_ic_ops->consys_ic_hw_vcn28_ctrl(1);
			}
			else
			{
